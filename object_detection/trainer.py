...
def train(create_tensor_dict_fn, create_model_fn, train_config, master, task,
  ...
  with tf.Graph().as_default():
    ...
    if (train_config.num_steps % save_step_interval == 0 or
        train_config.num_steps == train_config.num_steps):
      checkpoint_path = os.path.join(train_dir + '.ckpt')
      tf.logging.info('Saving to "%s-%d"', checkpoint_path, train_config.num_steps)
      saver.save(sess, checkpoint_path, global_step=train_config.num_steps)
